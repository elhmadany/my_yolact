...
def display_lincomb(proto_data, masks):
    ...
    for kdx in range(1):
        ...
        test = torch.sum(proto_data, -1).detach().numpy()
        ...
        for y in range(arr_h):
            for x in range(arr_w):
                ...
                if i == 0:
                    running_total = proto_data[:, :, idx[i]].detach().numpy() * coeffs_sort[i]
                else:
                    running_total += proto_data[:, :, idx[i]].detach().numpy() * coeffs_sort[i]
                ...
                arr_img[y*proto_h:(y+1)*proto_h, x*proto_w:(x+1)*proto_w] = (proto_data[:, :, idx[i]] / torch.max(proto_data[:, :, idx[i]])).detach().numpy()* coeffs_sort[i]
        ...
        # plt.imshow(test)
        # plt.show()
        plt.imshow(out_masks[:, :, jdx].detach().numpy())
        plt.show()
